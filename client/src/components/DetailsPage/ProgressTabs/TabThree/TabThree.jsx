  ...
  <>
    ...
    <div className={styles.tab_wrapper_three}>
      <div className={styles.card_holder}>
        {companyTabs.indexOf(tab) !== -1 ? (companyTabs.slice(companyTabs.indexOf(tab) + 1).length !== 0 ? companyTabs.slice(companyTabs.indexOf(tab) + 1).map((el, i) => {
          if (i === 0) {
            return (
              <>
                <span className={styles.whats_next_card} />
                <span className={[styles[cardDepth[i]], styles[stylesArr[companyTabs.indexOf(tab) + 1]]].join(' ')} />
              </>
            );
          }
          return <span className={[styles[cardDepth[i]], styles[companyTabs.indexOf(tab) + 1]].join(' ')} />;
        }) : <span className={styles.whats_next_card} />) : ''}
